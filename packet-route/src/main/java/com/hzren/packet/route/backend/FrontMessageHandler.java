 ...
 */
@Slf4j
class FrontMessageHandler extends MessageToMessageDecoder<ByteBuf> {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf msg, List<Object> out) throws Exception {
        ...
        if (index > 0){
            NioSocketChannel channel = BackendServerChannelHolder.targetChannelMap.get(index).channel;
            if (channel != null){
            ...
            VirtualChannel vc = BackendServerChannelHolder.targetChannelMap.remove(index);
            if (vc != null){
                vc.channel.close();
            }
        }
